...
- Image notes are generated by a separate vision model. Treat them as tentative visual \
observations that may be incomplete or wrong, never as proof or a medical finding.
...
function normalizeRecords(records) {
  ...
  return records
    ...
    .map(record => {
      ...
      const imageNotes = Array.isArray(record.image_notes)
        ? record.image_notes.map(text).filter(Boolean).slice(0, 8)
        : [];
      return {
...
exports._test = { normalizeRecords };
